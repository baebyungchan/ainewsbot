    ...
    *,
    ...
    for entry in feed.entries:
        ...
        if not summary and entry.get("media_description"):
            summary = strip_html(entry.get("media_description"))
        if ai_only and not is_ai_related(title, summary[:600]):
            continue
        items.append(
            Item(
...
async def fetch_all(client: httpx.AsyncClient, settings: Settings, report: dict) -> list[Item]:
    async def one(feed: dict) -> list[Item]:
        name = feed["name"]
        try:
            resp = await client.get(feed["url"], headers={"User-Agent": BROWSER_UA})
            ...
            return parse_feed(
                ...
                ai_only=feed.get("ai_only", False),
                icon=feed.get("icon", "📰"),
            )
        except Exception as exc:  # noqa: BLE001
            logger.warning("rss %s failed: %s", name, exc)
            report.setdefault("rss_errors", []).append(f"{name}: {str(exc).splitlines()[0]}")
            return []

    results = await asyncio.gather(*(one(f) for f in RSS_FEEDS))
